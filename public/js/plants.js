...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const createNewRow = plant => {
    ...
    const fetchPlantImages = plant => {
      fetch(`/api/images/${plant.id}`, {
        ...
        .then(response => {
          ...
          image.setAttribute(
            'style',
            'max-height:200px;max-width:200px;width:100%;justify-content:end'
          )
    ...
    newPlantUser.textContent = `Created by: ${plant.User.name}`
  ...
  const handlePlantDelete = e => {
    const currentPlant = JSON.parse(e.target.parentElement.parentElement.dataset.plant)
